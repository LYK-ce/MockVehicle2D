...
执行期运动。Patrol/Coverage 默认保持原本车语义；可选 `coordination_id` 在任务首次激活
时冻结 `self + expected peer allowlist`。Patrol 按排序后的 `vehicle_id` 均匀旋转起始航点，
每车仍完整执行原路线和全部 `cycles`；Coverage 沿原矩形长轴均分连续子矩形，每车仍用
原蛇形路线和 Cooperative Goto。同组所有成员必须收到相同 ID 和任务参数；当前不支持
部分下发、动态成员或运行中重分配。该能力不增加中央任务分配器、共享地图或新的
P2P/motion-intent schema，也不把 peer evidence 写入 OwnMap。
...
耗时更长的扩展矩阵选择最困难的对向合流 Patrol、共享入口 Coverage，以及四车共享闭环
Patrol 的 3 轮固定相位，验证同 seed 重复、车辆声明反序以及 `50/250 ms` tick，因此默认
deselected：

```bash
.venv/bin/python -m pytest -p no:cacheprovider -q -m extended tests/test_episode.py \
  -k 'extended_matrix or coordinated_patrol_route'
```
...
{"type":"auto","seq":6,"action":"push","missions":[
  ...
  {"mission_id":"patrol-001","type":"patrol","frame_id":"global_map","waypoints":[{"x_m":20.0,"y_m":30.0},{"x_m":24.0,"y_m":30.0}],"cycles":2,"coordination_id":"fleet-patrol-001"},
...
- `patrol` 按给定航点执行有限轮次；`coverage` 从矩形左下角开始，沿长边生成蛇形
  ...
  队列位置。
- `patrol.coordination_id` 可省略；设置时，固定 expected 车队按排序后的 `vehicle_id`
  旋转起始航点，每车仍完整执行原航点和全部轮次。
- `coverage.coordination_id` 可省略；设置时，固定 expected 车队按排序后的 `vehicle_id`
  沿区域长轴分片。两类同组任务的所有成员必须收到相同 ID 和任务参数。
